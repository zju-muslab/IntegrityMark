...
def save_op(path, epoch, encoder, decoder, discriminator, en_de_op, robust_type):
    ...
    torch.save(
        ...
        os.path.join(path,"none-"+ encoder.name + "attack_type_" + str(robust_type) + "_ep_{}_{}.pth".format(epoch, now.strftime("%Y-%m-%d_%H_%M_%S"))),
    )

def save_multi_decoder_op(path, epoch, encoder, robust_decoder, discriminator, en_de_op, robust_type):
    if not os.path.exists(path): os.makedirs(path)
    now = datetime.datetime.now()
    torch.save(
        {
...
def save_ckpt(ckpt_dir, base_name, module_state_dict, epoch):
    if not os.path.exists(ckpt_dir): os.makedirs(ckpt_dir)
    now = datetime.datetime.now()
    torch.save(module_state_dict,
        os.path.join(ckpt_dir, base_name+"_ep_{}_{}.pth".format(epoch, now.strftime("%Y-%m-%d_%H_%M_%S"))),
    )
