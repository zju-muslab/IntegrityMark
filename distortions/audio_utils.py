...
def MP3_compress_with_bitrate(wav, sr, bitrate="64k"):
    """
    将音频张量压缩为MP3格式并重新读取
    
    参数:
        wav (torch.Tensor): 形状为[channels, samples]的音频张量
        sr (int): 采样率
        target_bitrate (str): MP3比特率，如"64k", "128k", "320k"等
    ...
    # 2. 用 pydub 加载 WAV，并导出为 MP3（指定比特率）
    ...
    # 3. 重新读取压缩后的 MP3
    # 注意：load函数参数顺序应为(filepath, [可选参数])
    compressed_wav, compressed_sr = torchaudio.load(mp3_buffer)
    
    return compressed_wav, compressed_sr
...
def get_mp3(wav_tensor: torch.Tensor, sr: int, bitrate: str = "128k") -> torch.Tensor:
    ...
    batch_size, channels, length = wav_tensor.shape
    # Flatten tensor for conversion and move to CPU
    wav_tensor_flat = wav_tensor.reshape(1, -1).cpu()

    original_length=length*batch_size
    # Convert to MP3 format with specified bitrate
    wav_tensor_flat, _ = MP3_compress_with_bitrate(wav_tensor_flat, sr, bitrate=bitrate)
    # Reshape back to original batch format and trim or pad if necessary
    
    compressed_length = wav_tensor_flat.shape[-1]
    if compressed_length > original_length:
        wav_tensor_flat = wav_tensor_flat[..., :original_length]  # Trim excess frames
    elif compressed_length < original_length:
        padding = torch.zeros(
            0, original_length - compressed_length, device=device
        )
        wav_tensor_flat = torch.cat((wav_tensor_flat, padding), dim=-1)  # Pad with zeros
...
def get_aac(
    ...
    # FFmpeg already handles encoder delay automatically, no manual skip needed
    compressed_length_flat = aac_tensor.shape[-1]

    # Trim excess frames from the end to preserve watermark
...
def get_opus(
    ...
    bitrate: str = "64k"
    "",
    encoder_delay: int = 312,
) -> torch.Tensor:
    """Converts a batch of audio tensors to Opus format and then back to tensors.

    This function first saves the input tensor batch as WAV files, then uses FFmpeg to convert
    these WAV files to Opus format (in OGG container). Finally, it loads the files back into tensors.

    Args:
        wav_tensor (torch.Tensor): A batch of audio files represented as a tensor.
                                   Shape should be (batch_size, channels, length).
        sr (int): Sampling rate of the audio.
        bitrate (str): Bitrate for Opus conversion, default is '64k'.
        encoder_delay (int): Encoder delay samples to skip, default is 312.

    Returns:
        torch.Tensor: Batch of audio files converted to Opus and back, with the same
                      shape as the input tensor.
    """
    import tempfile
    import subprocess
    ...
    match = re.search(r"\d+(\.\d+)?", bitrate)
    parsed_bitrate = (
        match.group() if match else "64"
    )  # Default to 64 if parsing fails

    # Flatten tensor for conversion and move to CPU
    wav_tensor_flat = wav_tensor.reshape(1, -1).cpu()

    # Use .ogg container for Opus (better compatibility)
    with tempfile.NamedTemporaryFile(
        suffix=".wav", delete=False
    ) as f_in, tempfile.NamedTemporaryFile(suffix=".ogg", delete=False) as f_out:
        input_path, output_path = f_in.name, f_out.name
    ...
    try:
        ...
        # Prepare FFmpeg command for Opus conversion (use OGG container)
        command = [
            "ffmpeg",
            "-y",
            "-i",
        ...
        # Load the Opus audio back into a tensor
        opus_tensor, _ = torchaudio.load(output_path)

    except Exception as exc:
        raise RuntimeError(
            f"Failed to run Opus compression. "
            f"Make sure ffmpeg is installed with libopus support. "
            f"Check with: ffmpeg -encoders | grep opus"
        ) from exc
    finally:
        # Clean up temp files
    ...
    original_length_flat = batch_size * channels * original_length
    # FFmpeg already handles encoder delay (preskip) automatically, no manual skip needed
    compressed_length_flat = opus_tensor.shape[-1]

    # Trim excess frames from the end to preserve watermark
    if compressed_length_flat > original_length_flat:
        opus_tensor = opus_tensor[:, :original_length_flat]
    ...
    # Pad with zeros at the end if needed
        ...
        opus_tensor = torch.cat((opus_tensor, padding), dim=-1)

    # Reshape and adjust length to match original tensor
    wav_tensor = opus_tensor.reshape(batch_size, channels, -1)
    compressed_length = wav_tensor.shape[-1]

    assert compressed_length == original_length, (
        "Opus-compressed audio does not have the same frames as original one. "
        "One reason can be ffmpeg is not installed and used as proper backend "
        "for torchaudio, or the Opus encoder is not correct."
    )
    return wav_tensor.to(device)
